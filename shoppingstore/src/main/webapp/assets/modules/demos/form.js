...
layui.use(['form', 'admin', 'ax', 'upload', 'laydate', 'selectPlus'], function () {
    ...
    upload.render({
        ...
        , done: function (res) {
            $("#fileInputHidden").val(res.address);
    ...
    upload.render({
        ...
        , done: function (res) {
            $("#pictureInputHidden").val(res.address);
            Feng.success(res.msg);
        }
        , error: function () {
            Feng.error("上传图片失败！");
    ...
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/demo/addItem", function (data) {
            Feng.success("添加成功！");
    ...
    $("#backupPage").click(function () {
        window.location.href = Feng.ctxPath + "/demo/form";
    });
