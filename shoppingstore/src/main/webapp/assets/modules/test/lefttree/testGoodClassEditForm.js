...
layui.use(['layer', 'form', 'admin', 'ax', 'laydate'], function () {
    ...
    /** 初始化详情数据 */
    var ajax = new $ax(Feng.ctxPath + "/test/lefttree/testGoodClass/detail/" + Feng.getUrlParam("id"));
    var result = ajax.start();
    form.val('testGoodClassForm',result.data);

    /** 表单提交事件 */
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/test/lefttree/testGoodClass/edit", function (data) {
            if(data.code === 0){
                Feng.success("更新成功!");
